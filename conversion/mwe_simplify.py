...
def require_valid_tagging(tagging, simplify_gaps, simplify_weak):
    ...
    if simplify_gaps:
        assert re.match(r'^(O|B[IĪĨ]+)+$', tagging)
    if simplify_weak:
        assert re.match(r'^(O|B(o|bi+|I)*I+)+$', tagging)

IN_GAP_TAGS = {'o','b','i','ī','ĩ'}
I_TILDE = 'Ĩ'
i_TILDE = 'ĩ'
I_BAR = 'Ī'
i_BAR = 'ī'
...
if __name__=='__main__':
    ...
    for ln in fileinput.input(sys.argv[2:]):
        if not ln.strip(): 
            simplify(sentid, *zip(*sent), simplification=simplification, policy=policy)
            sent = []
            continue
        try:    # 9-column
            offset, tok, lemma, pos, tag, parent, strength, lbl, sentid = ln[:-1].split('\t')
            pos = [lemma, pos]
        except ValueError:  # 4-column
            tok, pos, tag, sentid = ln[:-1].split('\t')
            pos = [pos]
            #tok, pos1, pos2, tag, sentid = ln[:-1].split('\t')
        sent.append((tok, pos, tag))
    if sent:
        simplify(sentid, *zip(*sent), simplification=simplification, policy=policy)
